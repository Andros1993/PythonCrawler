...
def searchKeyworld(nextUrl, page, key_world):
    ...
    for text in li_list:
        ...
        if asin == 'B073XLR5HW':
            ...
            if str(text).find("a-spacing-none a-color-tertiary a-text-normal") == -1:
                print(key_world + "搜索排名在第" + str(page) + "页" + "，第" + str(row_position) + "行" + ",第" + str(list_position) + "个")
            else:
                print(key_world + "广告在第" + str(page) + "页" + "，第" + str(row_position) + "行" + ",第" + str(list_position) + "个")

for key_world_str in key_world_list:

    print("正在搜索关键词：" + key_world_str)
    key_world_str = key_world_str.replace(" ","+")
    base_url = "https://www.amazon.ca/s/ref=nb_sb_noss?url=search-alias%3Delectronics&field-keywords=" + key_world_str + "&rh=n%3A667823011%2Ck%3A" + key_world_str
    ...
    for i in range(2,10):
        ...
        try:
            _thread.start_new_thread(searchKeyworld, (nextUrl, i, key_world_str,))
        except:
            print("Error: 无法启动线程")
        time.sleep(1)
    time.sleep(1)